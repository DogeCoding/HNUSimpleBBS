...
class AlbumViewController: UIViewController, ListAdapterDataSource, UIScrollViewDelegate {
    ...
    fileprivate let category = "image_ppmm"
    var data: [BaseModel] = []
    ...
    func listAdapter(_ listAdapter: ListAdapter, sectionControllerFor object: Any) -> ListSectionController {
        if let obj = object as? BaseModel, obj.large_image?.url != nil {
            let ctr = AlbumViewSectionController()
            ctr.delegate = self
            return ctr
        }
...
extension AlbumViewController: BaseSectionControllerDelegate {
